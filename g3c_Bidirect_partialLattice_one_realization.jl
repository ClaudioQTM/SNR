...
"""
    partial_lattice_exact_N(rng, N; filling=0.1, Nrealization=N_max,
                            Nsites=round(Int, Nrealization/filling),
                            max_attempts=100_000)

Generate a finite lattice with exactly `Nrealization` occupied sites, then return
its first `N` ordered positions and the number of attempted patterns. Positions
are in units of the lattice spacing. Reinitializing `rng` with the same seed and
keeping `filling`, `Nrealization`, and `Nsites` fixed preserves every existing
position when `N` increases. The filling and site count refer to the full
realization; smaller lattices contain only its first `N` atoms.
"""
function partial_lattice_exact_N(rng::AbstractRNG, N::Integer;
                                filling::Real=0.1,
                                Nrealization::Integer=N_max,
                                Nsites::Integer=round(Int, Nrealization/filling),
                                max_attempts::Integer=100_000)
    0 < filling < 1 || throw(ArgumentError("filling must lie strictly between 0 and 1"))
    1 <= N <= Nrealization <= Nsites || throw(ArgumentError("require 1 <= N <= Nrealization <= Nsites"))
    max_attempts > 0 || throw(ArgumentError("max_attempts must be positive"))

    for attempt in 1:max_attempts
        occupied = rand(rng, Bernoulli(filling), Nsites)
        if count(occupied) == Nrealization
            return findall(occupied)[1:N], attempt
        end
    end
    error("No pattern with Nrealization=$Nrealization among $Nsites sites after $max_attempts attempts")
end
...
function g3c(N::Integer,ϵ;
    ...
    # Recreate the same full realization for each N and each value of ϵ.
    z, attempts = partial_lattice_exact_N(MersenneTwister(seed), N;
        filling=filling, Nrealization=Nrealization, Nsites=Nsites,
