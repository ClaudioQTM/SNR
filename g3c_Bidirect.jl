...
if N == 1 
    ...
    sum2 += ϵ/2 * sum(σp_full[l]*σm_full[j]*exp(2*1im*k_0*(z[j]-z[l]))-σp_full[j]*σm_full[l]*exp(-2*1im*k_0*(z[j]-z[l])) for l in 1:N for j in 1:l-1)
end
...
term5 = ϵ * D(sum(σm_full[i]*exp(2im*k_0*z[i]) for i in 1:N))
L += term5
term5 = nothing
